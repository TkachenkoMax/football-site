...
<body>
	...
	<img class="show" src="images/down.png" width="50px" height="50px"/>
		<div class="category-head" id="cl-head">
		</div>
		<div class="category-left">
			<span class="additional-text">Лента новостей</span>
			<div class="list-of-news">
				<?php 
				$news = mysqli_query($connect,"SELECT id_news, title, description, date, photo FROM news WHERE category = 'Лига чемпионов' ORDER BY id_news DESC");
				...
				<div class="category-controls">
					<?php if($count!=0) { ?> <a href="championsleague.php?page=1">На первую</a> <?php } ?>
					<?php if($page!=1) { ?> <a href="championsleague.php?page=<?php echo $page-1 ?>"><</a><?php } ?>
					<?php if($page+1<=ceil($count/3)) { ?> <a href="championsleague.php?page=<?php echo $page+1 ?>">></a><?php } ?>
					<?php if($count!=0) { ?> <a href="championsleague.php?page=<?php echo ceil($count/3) ?>">На последнюю</a> <?php } ?>
				</div>
			</div>
		</div>
		<?php require('championsleague_tables.php') ?>
	</div>
